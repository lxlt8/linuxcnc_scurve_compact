...
typedef struct {
    hal_bit_t *home_extern;  // use extern homing.
...
static int base_make_joint_home_pins(int id,int njoints)
{
    ...
    for (jno = 0; jno < njoints; jno++) {
        ...
        retval += hal_pin_bit_newf(HAL_IN, &(addr->home_extern), id,
                                   "joint.%d.home-extern", jno);
...
static int base_1joint_state_machine(int joint_num)
{
    ...
    bool home_extern = *joint_home_data->jhd[joint_num].home_extern;
    ...
    do {
        ...
        case HOME_INDEX_SEARCH_WAIT:
            ...
            if(!home_extern){
                ...
                joint->free_tp.curr_pos = joint->pos_cmd =  joint->pos_fb;
            }
            break;
