...
inline double scurve_scurve_time_period(struct scurve_period *p) {
...
inline double scurve_time_all_periods(struct scurve_data *data) {
    return scurve_scurve_time_period(&data->c0) + scurve_scurve_time_period(&data->c1) +
            scurve_scurve_time_period(&data->c2) + scurve_scurve_time_period(&data->c3) +
            scurve_scurve_time_period(&data->c4);
}
...
inline int scurve_update(struct scurve_data *data) {
    ...
    double length_tollerance = 1e-3;
    data->tarvel=tollerance ;
    ...
    // Stop to reach endvel.
    if(data->pd.need_stop==1){
        scurve_stop_lenght_time(data,&data->pd.stopdist,&data->pd.stoptime);
...
inline void scurve_solver_update(struct scurve_data *data) {
    ...
    if (data->curtime < scurve_scurve_time_period(&data->c0)) {
    ...
    if (data->curtime > scurve_scurve_time_period(&data->c0) + scurve_scurve_time_period(&data->c1) &&
            data->curtime < scurve_scurve_time_period(&data->c0) + scurve_scurve_time_period(&data->c1) + scurve_time_period(&data->c2)) {
    ...
    if (data->curtime >= scurve_scurve_time_period(&data->c0) + scurve_scurve_time_period(&data->c1) + scurve_time_period(&data->c2) &&
            data->curtime <= scurve_scurve_time_period(&data->c0) + scurve_time_period(&data->c1) + scurve_time_period(&data->c2) + scurve_time_period(&data->c3)) {
...
inline void scurve_t1_t3_custom_as(double jm,
    ...
    double ve = endvel;
    double vh = (vo + ve) / 2;
    double t1 = sqrt(2 * (vh - vo) / jm);

    *as = jm * t1;
}

int scurve_solve_errors(struct scurve_data *data){

    if(scurve_has_nan(data)){
        scurve_solver_build_curve(data);
        scurve_solver_update(data);
        return 1;
    }
...
int scurve_has_nan(const struct scurve_data *data) {
    if (data == NULL) {
    ...
    if (isnan(data->pd.stopdist) || isnan(data->pd.stoptime) || isnan(data->pd.overshoot)) {
        printf("has nan. \n");
        return 1;
    }
