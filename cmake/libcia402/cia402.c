...
void cia_read_all();
void cia_write_all();
cia_joint_data_t *jd = NULL;  // Initialize to NULL
...
int rtapi_app_main(void) {
    ...
    // Add read function.
    int r = hal_export_funct("cia402.read-all", cia_read_all, NULL, 0, 0, comp_idx);
    if(r){
        hal_exit(comp_idx);
    ...
    // Add write function.
    r = hal_export_funct("cia402.write-all", cia_write_all, NULL, 0, 0, comp_idx);
    if(r){
        hal_exit(comp_idx);
    ...
    jd = cia_allocate_joint_data(count);
    if (!jd) {
        hal_exit(comp_idx);
        return -1;
    }
    ...
    // Add hal pins.
    r = cia_setup_pins(jd, count, comp_idx);
    if(r) {
        jd = NULL;
        hal_exit(comp_idx);
        return r;
...
void rtapi_app_exit(void) {
    ...
    for(int i = 0; i < count; i++) {
        cia_joint_data_t *joint = &jd[i];
        cia_drive_shutdown(joint);
    }
    if(jd) {
        // hal_free(jd);
        jd = NULL;
...
void cia_drive_run(cia_joint_data_t *joint){
...
void drive_state_machine(cia_joint_data_t *joint){
    ...
    case READY_TO_SWITCH_ON:
        // Switch on.
        cia_drive_switch_on(joint);
        break;
    case SWITCH_ON:
        // Enable operation.
        cia_drive_enable_operation(joint);
        break;
...
void cia_write_all() {
    ...
    for(int i = 0; i < count; i++) {
        ...
        if(*joint->enable){
            ...
            cia_drive_write_position(joint);
            cia_drive_write_velocity(joint);
            cia_drive_write_torque(joint);
