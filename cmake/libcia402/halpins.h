 ...
 */
static joint_data_t* allocate_joint_data(int count) {
    if (count <= 0) {
        printf("Error: Invalid joint count %d\n", count);
        return NULL;
    }

    joint_data_t *jd = hal_malloc(sizeof(joint_data_t) * count);
    if (!jd) {
    ...
    memset(jd, 0, sizeof(joint_data_t) * count);

    printf("Allocated memory for %d joints\n", count);
    return jd;
}

static int setup_pins(joint_data_t *jd, int count, int comp_id){
